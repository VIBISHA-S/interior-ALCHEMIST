...
export default function Projects() {
  return (
    <section className="py-24 px-6 max-w-5xl mx-auto" id="portfolio">
      <div className="flex flex-col items-center text-center mb-12">
        <div className="inline-block px-3 py-1 mb-6 text-xs text-gray-400 border border-gray-800 rounded-full">
          Portfolio
        </div>
        <h2 className="text-3xl md:text-5xl font-medium leading-tight">A showcase of<br/>our recent projects</h2>
      </div>
      ...
      <div className="space-y-6">
        {projects.map((project, i) => (
          <motion.div 
            ...
            className="bg-[#111111] border border-gray-800/50 rounded-[32px] p-6 md:p-8 flex flex-col md:flex-row items-center gap-8 md:gap-10"
          >
            <div className="flex-1 space-y-4 order-2 md:order-1">
              <h3 className="text-2xl md:text-3xl font-medium leading-tight">{project.title}</h3>
            ...
            <div className="flex-1 w-full flex justify-center md:justify-end order-1 md:order-2">
              <div className="relative w-full max-w-[340px] aspect-[4/3] rounded-[2rem] overflow-hidden group">
                <Image 
                  src={project.image}
                  alt={project.title}
                  fill
